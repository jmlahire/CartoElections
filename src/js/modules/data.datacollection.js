...
class DataCollection {
     ...
     */
    load(file, options={}) {
        const defaultOptions = { delimiter: ',', mapper: d3.autoType, primary:undefined };
        this.ready=new Promise((resolve, reject) => {
            this.file=file;
            this.options={...defaultOptions,...options};
            d3.text(`${this.file}`)
                .then((dataset) => {
                    dataset = d3.dsvFormat(this.options.delimiter)
                                .parse(dataset)
    ...
    push(dataset,keyName='id') {
        const idKeys=['id','ID','insee','INSEE'];
        ...
        this.primaryKey = this.primaryKey || this.keys.filter( k => idKeys.includes(k) ).shift();
        return this;
    }
        ...
        if (type.toLowerCase() === 'map') {
            ...
            this.toGroups(primaryKey)
                .forEach( d => {
                    let value=d[1];
                    value.forEach((v)=> {
                        if (v.hasOwnProperty(primaryKey))  delete (v[primaryKey])
                    } );
                    this.datasetMap.set(d[0],value);
                } );
            return this.datasetMap;
        }
